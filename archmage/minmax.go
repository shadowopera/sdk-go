...
func (mm *MinMax[T]) Sample(rng *rand.Rand) T {
	...
	case *int:
		...
		v := xMin + rng.IntN(xMax-xMin+1)
		return *(*T)(unsafe.Pointer(&v))

	case *int8:
		xMin := *(*int8)(unsafe.Pointer(&mm.Min))
		xMax := *(*int8)(unsafe.Pointer(&mm.Max))
		v := int8(int64(xMin) + rng.Int64N(int64(xMax)-int64(xMin)+1))
		return *(*T)(unsafe.Pointer(&v))

	case *int16:
		xMin := *(*int16)(unsafe.Pointer(&mm.Min))
		xMax := *(*int16)(unsafe.Pointer(&mm.Max))
		v := int16(int64(xMin) + rng.Int64N(int64(xMax)-int64(xMin)+1))
		return *(*T)(unsafe.Pointer(&v))

	case *int32:
		xMin := *(*int32)(unsafe.Pointer(&mm.Min))
		xMax := *(*int32)(unsafe.Pointer(&mm.Max))
		v := int32(int64(xMin) + rng.Int64N(int64(xMax)-int64(xMin)+1))
		return *(*T)(unsafe.Pointer(&v))

	case *int64:
		xMin := *(*int64)(unsafe.Pointer(&mm.Min))
		xMax := *(*int64)(unsafe.Pointer(&mm.Max))
		v := xMin + rng.Int64N(xMax-xMin+1)
		return *(*T)(unsafe.Pointer(&v))

	case *uint:
		xMin := *(*uint)(unsafe.Pointer(&mm.Min))
		xMax := *(*uint)(unsafe.Pointer(&mm.Max))
		v := xMin + rng.UintN(xMax-xMin+1)
		return *(*T)(unsafe.Pointer(&v))

	case *uint8:
		xMin := *(*uint8)(unsafe.Pointer(&mm.Min))
		xMax := *(*uint8)(unsafe.Pointer(&mm.Max))
		v := uint8(uint(xMin) + rng.UintN(uint(xMax)-uint(xMin)+1))
		return *(*T)(unsafe.Pointer(&v))

	case *uint16:
		xMin := *(*uint16)(unsafe.Pointer(&mm.Min))
		xMax := *(*uint16)(unsafe.Pointer(&mm.Max))
		v := uint16(uint(xMin) + rng.UintN(uint(xMax)-uint(xMin)+1))
		return *(*T)(unsafe.Pointer(&v))

	case *uint32:
		xMin := *(*uint32)(unsafe.Pointer(&mm.Min))
		xMax := *(*uint32)(unsafe.Pointer(&mm.Max))
		v := uint32(uint(xMin) + rng.UintN(uint(xMax)-uint(xMin)+1))
		return *(*T)(unsafe.Pointer(&v))

	case *uint64:
		xMin := *(*uint64)(unsafe.Pointer(&mm.Min))
		xMax := *(*uint64)(unsafe.Pointer(&mm.Max))
		v := xMin + rng.Uint64N(xMax-xMin+1)
		return *(*T)(unsafe.Pointer(&v))

	case *float32:
		const maxPeriod32 = 1 << 24
		const divisor32 float32 = 1.0 / maxPeriod32
		r32 := float32(rng.Uint32N(maxPeriod32+1)) * divisor32

		xMin := *(*float32)(unsafe.Pointer(&mm.Min))
		xMax := *(*float32)(unsafe.Pointer(&mm.Max))
		v := xMin + r32*(xMax-xMin)
		return *(*T)(unsafe.Pointer(&v))

	case *float64:
		const maxPeriod64 = 1 << 53
		const divisor64 float64 = 1.0 / maxPeriod64
		r64 := float64(rng.Uint64N(maxPeriod64+1)) * divisor64
